/**
 ...
 Here's a breakdown of how this process typically works:
     ...
     5. The access token is then used by LinkedIn to make authenticated API calls to the authentication provider to retrieve user information. This allows LinkedIn to access the user's data without requiring the user to re-enter their credentials for each request.
 ...
 E. TODO [Kafka (Message Queue)]:

 Apache Kafka is a distributed event streaming platform designed to handle high-throughput, low-latency data streaming across distributed systems. Originally developed by LinkedIn and now maintained by the Apache Software Foundation, Kafka is widely used for building real-time data pipelines, messaging systems, and streaming applications.
 Kafka’s primary purpose is to handle event streams. It can stream real-time data across distributed systems and integrate with tools like Kafka Streams or ksqlDB to process, aggregate, and analyze data in real time. Message queues typically focus on simple message delivery and do not have these stream processing capabilities built-in.

 1. Basic Concepts:
     - Topics: Think of these as categories or feed names where data is stored and published. Like a folder for specific types of messages (e.g., "user_signups", "page_views")
     - Partitions: Topics are split into partitions for scalability. Each partition is an ordered, immutable sequence of messages
     - Brokers: The Kafka servers that store and manage the topics/partitions
     - ZooKeeper: Coordinates the Kafka cluster and maintains configuration
     - Messages: The actual data units being sent, each with a key, value, timestamp
 ...
 2. Producer/Consumer Patterns:
     ...
     - Consumer Groups: Groups of consumers that divide up the reading of topics
     - Offsets: Bookmarks that track what messages have been read
     - At-least-once/Exactly-once delivery semantics

 3. Pub/Sub (Publish-Subscribe) and Queue Models:
     * Kafka can operate in both queue and publish-subscribe models:
         - In the publish-subscribe model, all consumers receive all messages (like traditional pub-sub systems).
         - In the queue model, consumers in a consumer group divide the messages among themselves, with each message being processed by only one consumer (similar to traditional message queues like RabbitMQ).

 4. Stream Processing:
     - Kafka Streams: Library for building streaming applications
     - Real-time processing: Process data as it arrives
     - Stateful operations: Maintain aggregations and counts
     - Window operations: Process data within time windows
     - Join operations: Combine different streams of data

 5. Configuration and Setup:
     - Cluster setup: Multiple brokers for fault tolerance
     - Replication factor: Number of copies of each partition
     - Retention policies: How long to keep messages
     - Compaction: How to handle updates to keyed data
     - Performance tuning: Batch sizes, compression, etc.

 6. Common Use Cases:
     - Event sourcing: Recording all state changes
     ...
     - Activity tracking: User behavior analysis
     - Decoupling systems: Message passing between applications

 7. When Kafka Might Not Be the Best Choice:
     - Low latency with a focus on individual message processing, acknowledgment, and delivery guarantees may be better handled by traditional message queues like RabbitMQ.
     - Complex routing scenarios that require sophisticated message routing or prioritization might be easier to implement with other message brokers.

 8. Some important architectural considerations:
     * Scalability:
         - Horizontal scaling by adding more brokers
         - Partition parallelism for better throughput
         - Consumer groups for parallel processing
     * Reliability:
         - Replication for fault tolerance
         - Leader/follower model for partitions
         - Acknowledgment levels for ensuring delivery
     * Performance:
         - Sequential disk I/O
 ...
 9. Kafka Message Flow:
     - Producers send messages (events) to a topic.
     - The topic is divided into partitions, and each message is assigned a unique offset within its partition.
     - Kafka brokers distribute partitions across the cluster.
     - Consumers (or consumer groups) subscribe to topics, reading messages in order from the partitions they are assigned to.
     - Kafka retains the messages based on a retention policy (time-based or size-based), allowing for replay of messages even after they’ve been consumed.

 10. Kafka vs. Traditional Message Queues:
     - Kafka: Distributed, durable, scalable, persistent (messages are not removed after consumption), better suited for high-throughput, real-time streaming data.
     - Traditional Message Queues (e.g., RabbitMQ): Typically offers lower-latency message delivery, more complex routing options, and message acknowledgment with more focus on immediate message consumption.
